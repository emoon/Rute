...
impl<'a> From<WrapperRcOwn> for Cursor<'a> {
    fn from(t: WrapperRcOwn) -> Self {
        let mut data = RUCursor {
            qt_data: ::std::ptr::null(),
            host_data: ::std::ptr::null(),
            all_funcs: t.all_funcs as *const RUCursorAllFuncs,
        };

        if t.owned {
            data.host_data = t.data as *const RUBase;
            Cursor::new_from_rc(data)
        } else {
            data.qt_data = t.data as *const RUBase;
            Cursor::new_from_temporary(data)
        }
    }
}
