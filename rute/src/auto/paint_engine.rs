...
impl<'a> From<WrapperRcOwn> for PaintEngine<'a> {
    fn from(t: WrapperRcOwn) -> Self {
        let mut data = RUPaintEngine {
            qt_data: ::std::ptr::null(),
            host_data: ::std::ptr::null(),
            all_funcs: t.all_funcs as *const RUPaintEngineAllFuncs,
        };

        if t.owned {
            data.host_data = t.data as *const RUBase;
            PaintEngine::new_from_rc(data)
        } else {
            data.qt_data = t.data as *const RUBase;
            PaintEngine::new_from_temporary(data)
        }
    }
}
